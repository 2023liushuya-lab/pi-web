...
function PiAgentTitle() {
  ...
  const target = showVersion ? `${process.env.NEXT_PUBLIC_APP_VERSION ?? "0.0.0"}p${process.env.NEXT_PUBLIC_PI_VERSION ?? "0.0.0"}` : t("app.title", "芫荽 Pi Web");
          ...
          {(["sessions", "resources"] as const).map((tab) => (
            <button
              ...
              style={{
                ...
                fontSize: 12,
                color: sidebarTab === tab ? "var(--text)" : "var(--text-muted)",
                cursor: "pointer",
      ...
      {confirmDelete ? (
        ...
        <>
          <div style={{ flex: 1, minWidth: 0, fontSize: 12, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {t("delete.question")} <span style={{ fontWeight: 600 }}>&ldquo;{title.slice(0, 22)}{title.length > 22 ? "…" : ""}&rdquo;</span>?
          </div>
